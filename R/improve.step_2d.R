...
"improve.step_2d.hr" <- function(gap,knotDist,radii,dists,explData,num,response,knotgrid,maxIterations,fitnessMeasure, point,knotPoint,position,aR,BIC,track,out.lm,improveNudge,tol=0,baseModel,radiusIndices,models, interactionTerm, data, initDisp, cv.opts, basis, subselect, submodel,submodels, model_sub){
  if (isS4(baseModel)){
    attributes(baseModel@misc$formula)$.Environment<-environment()
  } else {
    attributes(baseModel$formula)$.Environment<-environment()
    ...
    for (i in 1:num) {
      ...
      for (j in nhbrs){
        ...
        if ((length(otherKnots)==0) || ( min(knotDist[j,otherKnots])>=gap)) {
          ...
          output<-fit.thinPlate_2d.hr(fitnessMeasure, dists,tempR,radii,baseModel,radiusIndices,models, fitStat, interactionTerm, data, initDisp, cv.opts, basis,subselect,submodel,submodels)
          initModel<-output$currentModel
          models<-output$models
          temp_sub <- output$subMod
          submodels <- output$submodels
          initBIC <- output$fitStat + temp_sub$subStat
          #get.measure_2d(fitnessMeasure,fitStat,initModel, data,  dists, tempR,radii,radiusIndices, initDisp)$fitStat
