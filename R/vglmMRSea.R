...
vglmMRSea<-function(formula, family = stop("argument 'family' needs to be assigned"),
                    data, weights = NULL, subset = NULL, na.action = na.fail,
  ...
  if (exists("panels")) {
    slot(answer, "panels") <- panels
  } else {
    slot(answer, "panels") <- NULL
  }
  
  answer
}
attr(vglmMRSea, "smart") <- TRUE
...
shadowvglm <-
  function(formula,
    ...
    list(Xm2 = x, Ym2 = y, call = ocall, Terms2 = mt, splineParams=splineParams, varshortnames=varshortnames)
  }
