...
"exchange.step_2d.hr" <- function(gap,knotDist,radii,dists,explData,response,knotgrid,maxIterations,fitnessMeasure, point,knotPoint,position,aR,BIC,track,out.lm,improveEx, maxKnots,tol=0,baseModel,radiusIndices,models, interactionTerm, data, initDisp, cv.opts, basis, subselect, submodel, submodels){
  ...
  while ( (improve) & (fuse < maxIterations) ) {
    ...
    if (length(legPos)>0) {
      ...
      #index<-which.min(abs(new[,1])+abs(new[,2]))
      if (!(any(knotDist[point[index[1]],aR]<gap))) {
        output <- move.knot_2D.hr(radii,dists,explData,index,fitnessMeasure,BIC,aR,point,
                                  response,knotgrid,out.lm,improve,improveEx, track,
                                  maxKnots,tol,baseModel,radiusIndices,models, interactionTerm, data, initDisp, cv.opts, basis, subselect, submodel, submodels)
        print("mk")
        improve <- output$improve
        improveEx <- output$improveEx
        mod_sub <- thinModels.hr(output$models, output$submodels)
        models <- mod_sub$models
        submodels <- mod_sub$submodels
        submod <- output$submod
        if (1 - improve) break
        out.lm<-output$out.lm
        track <- output$track
        tempKnot <- output$tempKnot
  ...
  # cat('Current Fit out: ', BIC, '\n')
  list(point=point,knotPoint=knotPoint,position=position,aR=aR,BIC=BIC,track=track,out.lm=out.lm,
       improveEx=improveEx,radiusIndices=radiusIndices,models=models,submod=submod,submodels=submodels)
}
